...
def train_model(model_fn, FIXED_PARAMETERS, even_check=False, model_type = ''):
    ...
    history = model.fit(X_train, y_train, FIXED_PARAMETERS['batchsize'], FIXED_PARAMETERS['epochs'], callbacks=callbacks, validation_data=(X_val, y_val), verbose = 0)



def evaluate_model(model_fn, FIXED_PARAMETERS, print_bool=True, model_type=''):
    (X_train, y_train), (X_val, y_val), (X_test, y_test), (X_sen, y_sen), metadata = prepare_dataset_merged_keras_combined(FIXED_PARAMETERS, agency_list='agency_list_train', data_string=model_type, print_bool=True)
...
def eval_train_model(model_fn, FIXED_PARAMETERS, print_bool=True, model_type=''):
    (X_train, y_train), (X_val, y_val), (X_test, y_test), (X_sen, y_sen), metadata = prepare_dataset_merged_keras_combined(FIXED_PARAMETERS, agency_list = 'agency_list_train', data_string = model_type, print_bool=True)
    num_classes = 2# len(np.unique(np.argmax(y_test, axis=-1)))
...
def eval_val_model(model_fn, FIXED_PARAMETERS, print_bool=True, model_type=''):
