...
def train_model(model_fn, FIXED_PARAMETERS, even_check=False, model_type = '', hospital=[]):
    (X_train, y_train), (X_val, y_val), (_, _), (_, _), metadata = prepare_dataset_merged_keras_combined(FIXED_PARAMETERS, agency_list='agency_list_train', data_string=model_type, print_bool=True)
    ...
    weights_path = './weights/sen_%s%s.h5' % (FIXED_PARAMETERS['agency_list_train'], model_type)
...
def eval_sen_model_v2(model_fn, FIXED_PARAMETERS, print_bool=True, model_type='', hospital=[], conditions = None):
    (_, _), (_, _), (_, _), (X_sen, y_sen), metadata = prepare_dataset_merged_keras_combined(FIXED_PARAMETERS, agency_list = 'agency_list_train', data_string = model_type, print_bool=False)
    num_classes = 2# len(np.unique(np.argmax(y_test, axis=-1)))
    ...
    if conditions != None:
        for c in conditions:
            bool_list = np.isin(X_sen[c[0]], c[1])
            X_sen = [X_sen[i][bool_list] for i in range(len(X_sen))]
            y_sen = y_sen[bool_list]
...
def eval_val_model(model_fn, FIXED_PARAMETERS, print_bool=True, model_type='', hospital=[]):
    (_, _), (X_val, y_val), (_, _), (_, _), metadata = prepare_dataset_merged_keras_combined(FIXED_PARAMETERS, agency_list = 'agency_list_train', data_string = model_type)
    num_classes = 2# len(np.unique(np.argmax(y_test, axis=-1)))
